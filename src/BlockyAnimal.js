...
let g_pokeTime = 0;
let g_nose_size = 2;
let g_color_1 = [0.1, 0.1, 0.1, 1.0];
let g_color_2 = [0.545, 0.271, 0.075, 1.0];
...
function updatePokeAnimation() {
  if (g_pokeAnimation) {
    ...
    g_tailAngle = 75 * Math.sin(g_pokeTime * 0.1); // tail movement

    if (g_pokeTime > 100) { // stop animation after 100 frames
      g_tailAnimation = false;
      g_pokeAnimation = false;
      g_tailAngle = 0; // reset tail
    }
    renderAllShapes(); // update scene
  }
}
...
function main() {
  ...
  g_startTime = performance.now() / 1000.0; // start time
  renderAllShapes(); // draw initial scene
  requestAnimationFrame(tick); // start loop
}

function tick() {
  g_seconds = performance.now() / 1000.0 - g_startTime; // calculate elapsed time
  updateAnimationAngles(); // update animated angles
  updatePokeAnimation(); // update poke effect
...
function renderAllShapes() {
  ...
  // Draw the body cube
  var body = new Cube();
  body.color = [0.1, 0.1, 0.1, 1.0]; // Black
  ...
  // Draw the tail
  var tail2 = new Cube();
  tail2.color = [0.36, 0.25, 0.20, 1.0]; // Dark Brown
  ...
  // Draw a neck
  var neck = new Cube();
  neck.color = [0.36, 0.25, 0.20, 1.0]; // Brown
  ...
  neck.matrix.scale(0.25, 0.20, 0.25);
  neck.matrix.translate(-0.5, 0, 0);
  neck.render();

  // Draw a head
  var head = new Cube();
  head.color = [0.1, 0.1, 0.1, 1.0]; // Black
  head.matrix = neckCoordinatesMat;
  head.matrix.translate(0, 0.20, -0.15);
  ...
  // Draw a nose
  var nose2 = new Cube();
  nose2.color = [0.35, 0.35, 0.35, 1.0]; // Deep Black
